...
mongoose.connect('mongodb+srv://user:user@cluster0.muoll.mongodb.net/projetDauphine?retryWrites=true&w=majority')
    .then(() => {
...
function verifyToken(req, res, next) {
    if (!req.headers.authorization) {
        return res.status(401).send('acces refuse1')
    }
    let token = req.headers.authorization.split(' ')[1]
    if (token === 'null') {
        return res.status(401).send('acces refuse2')
    }
    let payload = jwt.verify(token, 'myKey')
    if (!payload) {
        return res.status(401).send('acces refuse3')
    }
...
router.post('/login', (req, res) => {
    ...
    User.findOne({
        ...
        if (error) {
            ...
            if (!user) {
                res.status(401).send('pas le bon email')
            } else
                if (user.password !== userData.password) {
                    res.status(401).send('pas le bon mdp')
                }
                else {
...
//ici on rajoute verifyToken pour la couche middleware
